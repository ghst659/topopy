...
class Graph:
    ...
    def subgraph(self, request_hi=None, request_lo=None):
        ...
        return SubGraph(upper=set(e for e in start_hi if e in node_set),
                        nodes=node_set,
...
class Rabbit:
    def __init__(self, graph, operator):
        ...
        self._rspec_op = None
        self._rvar_pool = None
        self._rvar_stop = threading.Event()
        self._rvar_enter = {}
        self._rvar_leave = {}
        self._rvar_block = set()

    def completions(self):
        with self._lock:
            return set(self._rvar_leave.keys())

    def blocked(self):
        with self._lock:
            return set(self._rvar_block)

    def result(self, node):
        outcome = None
        with self._lock:
            future = self._rvar_enter.get(node, None)
            outcome = future.result() if future and future.done() else None
        return outcome

    def run(self, concurrency, timeout_limit, request_hi, request_lo, meta={}):
        with self._lock:
            self._rvar_stop.clear()
            self._rvar_enter.clear()
            self._rvar_leave.clear()
            self._rvar_block.clear()
        if self._op and self._dag and self._dag.size():
            with self._lock:
                self._rspec_meta = dict(meta)
                self._rspec_graph = self._dag.subgraph(request_hi, request_lo)
            with concurrent.futures.ThreadPoolExecutor(max_workers=concurrency) as self._rvar_pool:
                self._visit_multi(self._rspec_graph.upper)
                self._wait_done(timeout_limit)


    def _wait_done(self, timeout_limit):
        self._rvar_stop.wait(timeout_limit)

    def _visit_multi(self, nodes_to_visit):
        if nodes_to_visit:
            for n in (v for v in nodes_to_visit if v in self._rspec_graph.nodes):
                if not self._rvar_stop.is_set():
                    prereqs = set(v for v in self._dag.predecessors(n)
                                  if v in self._rspec_graph.nodes)
                    with self._lock:
                        completions = set(self._rvar_leave.keys())
                        if completions >= prereqs and n not in self._rvar_enter:
                            future = self._rvar_pool.submit(self._visit_single, n,
                                                            **self._rspec_meta)
                            self._rvar_enter[n] = future
    ...
    def _visit_single(self, node, **kwargs):
        ...
        try:
            ...
            self._rvar_leave[node] = time.time()
        with self._lock:
            concluded = self._rvar_block | set(self._rvar_leave.keys())
            if concluded >= self._rspec_graph.nodes:
                self._rvar_stop.set()
                # self._rvar_pool.shutdown()
            else:
                self._visit_multi(self._dag.successors(node))
        return result

    def _update_blocks(self, node):
        """Update the block set for NODE."""
        with self._lock:
            self._rvar_block |= (self._rspec_graph.nodes & self._dag.downstreams(node))
